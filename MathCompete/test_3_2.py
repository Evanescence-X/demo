...
def print_final_statistics(solution: List[MaterialInstance], orders: List[Order]):
    """打印最终统计结果"""
    # 计算总成本
    total_cost = sum(inst.material_type.cost for inst in solution)

    # 计算利用率
    utilization, loss_rate = calculate_utilization(solution)
    ...
    print("\n===== 最终统计结果 =====")
    print(f"{'总成本':<15}: {total_cost:.2f} 元")
    print(f"{'材料利用率':<15}: {utilization * 100:.2f}%")
    print(f"{'切割损失率':<15}: {loss_rate * 100:.2f}%")
    print(f"{'总收益':<15}: {total_revenue:.2f} 元")
    print(f"{'总利润':<15}: {profit:.2f} 元")

    # 详细成本分析
    print("\n===== 成本分析 =====")
    cost_by_type = {}
    for inst in solution:
        mat_type = inst.material_type.id
        if mat_type not in cost_by_type:
            cost_by_type[mat_type] = 0
        cost_by_type[mat_type] += inst.material_type.cost

    for mat_type, cost in cost_by_type.items():
        print(f"类型 {mat_type} 原材料总成本: {cost:.2f} 元")

    # 需求满足情况
    print("\n===== 需求满足情况 =====")
    demand_map = {seg.id: seg.demand for seg in all_segments}
    fulfilled_map = {seg.id: 0 for seg in all_segments}

    for inst in solution:
        for pos, seg in inst.cut_segments:
            fulfilled_map[seg.id] += 1

    for seg in all_segments:
        fulfilled = fulfilled_map[seg.id]
        demand = demand_map[seg.id]
        status = "✓" if fulfilled >= demand else f"✗ ({fulfilled}/{demand})"
        print(f"{seg.type} {seg.length:.1f}m: {status}")
...
def main():
    ...
    # 计算贪心解的结果
    print("\n===== 贪心算法初步结果 =====")
    print_final_statistics(greedy_result, orders)
    ...
    # 步骤3: 输出最终优化结果
    print("\n===== 遗传算法优化后结果 =====")
    print_final_statistics(best_solution, orders)
